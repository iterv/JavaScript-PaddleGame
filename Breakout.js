...
function start_game() {
	...
	requestAnimationFrame(draw);
        document.getElementById("scoreBoard").innerHTML = "Peli alkoi: " + gameStart + "<br> Pelien määrä: " + gameCount;
}
...
function showPlayingTime() {
	...
	ctx.fillText("Time elapsed: "+seconds+"s",10,790);
        document.getElementById("scoreBoard2").innerHTML = "Kesto: " + seconds + "s";
}
	...
	for (var i=0 ; i<blocks.length ; i++) {
		if (hitTest(ball.x,ball.y,ball.w,ball.w,blocks[i].x,blocks[i].y,blocks[i].w,blocks[i].h)) {
			...
			blocks.splice(i,1); // Poistaa i:nnen blockin taulukosta
                        document.getElementById("scoreBoard3").innerHTML = "Pisteet: " + score++;
			// all blocks gone
			if (blocks.length == 0) {
				alert("All blocks destroyed!");
